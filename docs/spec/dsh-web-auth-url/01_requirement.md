...
- 个人 Windows 模式固定绑定 `127.0.0.1`，不向局域网暴露 dsh 服务。
- 启动超时或 dsh 提前退出时清理子进程和认证状态，不留下“失败但仍运行”的半状态。
- 从 Harness 通过托盘返回控制面板时停留在控制台，不因已有服务运行而再次自动跳回 Harness。
...
5. 启动超时、提前退出、停止和重启后不会残留本应用托管的 dsh 进程或认证 URL。
6. 从 Harness 返回控制面板后保持控制台页面，手动点击“进入 Harness”仍可正常导航。
7. Rust 测试、前端构建和 Tauri 构建通过。
