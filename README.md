...
- **端口检测**：启动前探测 `127.0.0.1:3081` / `[::1]:3081`；已被占用时顶栏显示琥珀色
  状态点并直接加载现有实例，不会重复启动
- **控制台页（仪表盘）**：
  - 服务卡片：端口/URL（一键复制）、PID、运行时长、日志文件路径（一键打开文件夹）
  - 环境卡片：node / dsh 路径与缺失提示（含安装命令）、重新检测
  - 操作行：进入 Harness、重启服务（外部占用时禁用）、自检诊断（可复制报告）、
    检查 dsh 更新（npm view 对比本地版本，10 分钟缓存、离线静默降级）
...
- **设置页**（托盘「设置」或控制台头部入口直达）：
  ...
  - 外观：深浅主题（Mica + 页面配色）
  - 调试（开发版）：DevTools 自动打开；关于：版本、图标署名、配置文件路径
- **全屏**：托盘菜单「进入全屏 / 退出全屏」一键切换（文案随当前状态自动变化）
- **全屏快捷键**：`F11` 切换全屏、`Esc` 退出全屏；快捷键在应用控制台页与 Harness 界面均可用
  （解决主屏全屏时任务栏/托盘被盖住、无法退出的问题）；Esc 优先让给 Harness 自身使用，
  仅当页面未拦截且焦点不在输入控件内时退出全屏
- **整窗进入 Harness**：服务就绪后自动整窗导航到 dsh 输出的带 token 认证 URL（跨站 iframe
  不可行，改为顶层导航）；旧版 dsh 未输出认证 URL 时回退到基础地址
- **系统托盘**：关闭主窗口 → 最小化到托盘；托盘菜单（自上而下）：进入全屏 / 退出全屏、
  控制面板、功能设置、退出应用（「控制面板」与「功能设置」会从 Harness 界面导航回本地页；
  退出时先杀进程树）
...
```
xzy-dsh-desktop/
├── index.html                  # 前端入口（控制台/设置双视图，hash 路由）
├── src/
│   ├── ipc.js                  # Tauri invoke 封装（命令名常量）
│   ├── main.js                 # 入口：共享状态、hash 路由、轮询、启停、环境检测
│   ├── views-console.js        # 控制台视图：信息卡片、操作行、日志查看器、异常检测
│   ├── views-settings.js       # 设置视图：端口/自启/主题/DevTools/关于
│   ├── styles.css              # 深浅双主题样式
│   └── brand-icon.png          # 鲸鱼娘图标（64×64）
├── scripts/
...
- 关闭窗口 = 最小化到托盘（右下角图标）；托盘菜单（自上而下）：进入全屏 / 退出全屏、
  ...
  本地页对应视图；「退出应用」= 真正退出并强杀 dsh 进程；服务启停、状态查看均在控制台页完成
- 若 3081 已被占用（比如 Harness 已在别处运行），状态点显示为琥珀色并直接加载
  现有实例，不会重复启动
- 托盘菜单「进入全屏 / 退出全屏」：一键进入/退出全屏；窗口为系统原生标题栏，
  最小化/最大化/关闭由系统按钮负责，无自绘顶栏
...
修改后**重启应用生效**（端口/主题等也可在设置页修改，写入同一文件）：
