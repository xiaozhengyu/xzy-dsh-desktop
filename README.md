...
- **内嵌 Web UI**：运行中自动在 iframe 中加载 `http://127.0.0.1:3080`（已验证 dsh 的
  Web 服务不发送 X-Frame-Options / CSP frame-ancestors，可嵌入）；也可开独立窗口
- **内存可调**：标题栏「自动加载」开关（默认开，符合任务书；关闭后运行中不自动加载
  Web 界面、按需手动加载，可释放重型渲染进程，显著降低常驻内存）
...
- 若 3080 已被占用（比如 Harness 已在别处运行），会显示黄色“Harness 可能已在运行”，
  并直接加载现有实例，不会重复启动
- 顶部「自动加载」开关：关闭后运行中不会自动渲染 Web UI（省内存），需要时点占位区的
  「加载 Web 界面」即可；偏好会自动记住
- 界面左下角「日志」可查看 dsh 服务输出，便于排错
...
因此：**自动加载开启**时整棵进程树约 450–600 MB 工作集；**关闭「自动加载」**后不渲染
Harness UI，可省下约 130–200 MB。150 MB 目标在“WebView2 + 内嵌完整 Harness UI”前提下
无法达成——这是 WebView2 多进程体系 + 重型前端决定的，Tauri 已是同场景最轻方案
（Electron 同类场景通常 700 MB+）。若追求极致内存，建议关闭自动加载、按需打开独立窗口。
